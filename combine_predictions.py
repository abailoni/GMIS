...
def parse_args():
  ...
  parser.add_argument('--h5_inner_path_result_file', type=str,
                      help="h5_inner_path used to get the segmentation")
...
def post_processing(args, process_id, counter, demo_files_queue,
  ...
  while True:
    ...
    # img = os.path.basename(demo_file)
    # dir = os.path.dirname(demo_file)
    # images.append(img)
    # continue
    ...
    for mask_i, mask in enumerate(mask_list):
      ...
      for j in range(1, connective_area_num + 1):
        ...
        inst_out_h5_file = inst_input_bin_file.replace(
            '.input.bin', '.output.h5')
        tmp_inst_out_conf_file = inst_input_bin_file.replace(
            '.input.bin', '.inst.confidence.h5')
        # tmp_inst_out_conf_txt_file = inst_input_bin_file.replace(
        #     '.input.bin', '.inst.confidence.txt')

        if not os.path.exists(inst_out_h5_file):
          print(inst_out_h5_file, " NOT COMPLETE")
          not_complete = True
          break

        with h5py.File(inst_out_h5_file, 'r') as f:
          tmp_inst_result_image = f[args.h5_inner_path_result_file][:]

        with h5py.File(tmp_inst_out_conf_file, 'r') as f:
          tmp_confidence_arr = f[args.h5_inner_path_result_file][:]
...
def main():
  ...
  # Update dirs depending on h5_inner_path:
  args.out_dir = os.path.join(args.out_dir, args.h5_inner_path_result_file)
  args.evaluation_result_out_dir = os.path.join(args.evaluation_result_out_dir, args.h5_inner_path_result_file)
